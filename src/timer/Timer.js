...
const Timer = ({ setHide }) => {
    ...
    useEffect(() => {
        ...
        if (time !== startingTime) {
            ...
            setTimeout(() => {
                //  Checking to make sure the time has not run out (AKA Checking to see if time is equals to zero)
                if (time > 0) {
                    // Updating the vlaue for the "time" state variable which causes the "Timer" component to re-render and also causes the "useEffect" function to run
                    setTime(time - 1);
